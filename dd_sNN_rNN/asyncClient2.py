...
async def sendSNNRequest(session, frames):
    i = random.randint(0,len(SPM_URLs)-2)
...
async def sendRNNRequest(metadata, session, frames, target):
    i = random.randint(0,len(REFM_URLs)-1)
...
def main():
    ...
    loop = asyncio.get_event_loop()
    results = loop.run_until_complete(sendAllRequests(executor, metadata, X_new, origX_new, c_low, c_high, target))  
